...
execute as @a[tag=readyforchange,scores={armor_change=1}] at @s if entity @e[type=minecraft:armor_stand,tag=quickarmor,distance=..0.5] at @e[type=armor_stand,distance=..1] run setblock ~ ~2 ~ minecraft:shulker_box
execute as @a[tag=readyforchange,scores={armor_change=1..}] at @s if entity @e[type=minecraft:armor_stand,tag=quickarmor,distance=..0.5] run function basicplus:quickchange/changer
