...
#Chang
execute as @a store success score @s afk_rotation if score @s rotation = @s rotation_check
...
execute as @a[scores={afk_rotation=1}] run scoreboard players add @s afk_timer 1
execute as @a[scores={afk_rotation=0}] run scoreboard players reset @s afk_timer
...
execute as @a[tag=afk,scores={afk_rotation=0}] run tellraw @a ["",{"selector":"@s","color":"gold"},{"text":" ist nun nicht mehr ","color":"gold"},{"text":"AFK","color":"gold","hoverEvent":{"action":"show_text","value":{"text":"","extra":[{"text":"Away From Keyboard","color":"gold"}]}}}]
execute as @a[tag=afk,scores={afk_rotation=0}] run tag @s remove afk
