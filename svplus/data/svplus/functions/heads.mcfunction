...
execute as @a[name=IFloooI,scores={life0},tag=!bek] at @s run summon item ~ ~ ~ {Item:{id:"minecraft:player_head",Count:1b,tag:{SkullOwner:{Name:"IFloooI"}}}}
execute as @a[name=IFloooI,scores={life0},tag=!bek] run tag IFloooI add bek
execute as @a[name=IFloooI,scores={life1..,},tag=bek] run tag IFloooI remove bek
