...
execute as @e[type=item,tag=craft_safezone1] at @s run data merge entity @s {Motion:[0.0d,0.3d,0.0d],Tags:[],Item:{id:"minecraft:shield",Count:1b,tag:{display:{Name:"{\"text\":\"Safe_Zone\"}",Lore:["Dieses Schild erzeugt eine sichere Zone!"]}}}}
execute as @e[nbt={OnGround:1b,Item:{id:"minecraft:shield",Count:1b,tag:{display:{Name:"{\"text\":\"Safe_Zone\"}",Lore:["Dieses Schild erzeugt eine sichere Zone!"]}}}}] at @s run function entplus:safezone2
