...
execute as @e[tag=efurnace,tag=!owned,tag=got_uuid] if score @p uuid = @s uuid run scoreboard players remove @p energy 2
execute as @e[tag=efurnace,tag=owned, tag=got_uuid] at @s if score @r uuid = @s uuid store result score @s energy run scoreboard players get @p energy
execute as @e[tag=efurnace,tag=!owned,tag=got_uuid] if score @p uuid = @s uuid run tag @s add owned
